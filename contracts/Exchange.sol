...
contract Exchange is owned {
    ...
    // event for transaction fee receipt
    event TransactionFeeApplied(uint indexed _symbolIndex, address indexed _from, uint _amount, uint _fee, uint _timestamp);
    ...
    function addToken(string symbolName, address erc20TokenAddress) public onlyowner {
        ...
        require(symbolNameMaxIndex + 1 > symbolNameMaxIndex);
        symbolNameMaxIndex++;

        tokens[symbolNameMaxIndex].symbolName = symbolName;
        tokens[symbolNameMaxIndex].tokenContract = erc20TokenAddress;
        TokenAddedToSystem(symbolNameMaxIndex, symbolName, now);
    }
    ...
    function getSymbolIndex(string symbolName) view internal returns (uint8) {
        for (uint8 i = 1; i <= symbolNameMaxIndex; i++) {
    ...
    function getSymbolIndexOrThrow(string symbolName) view public returns (uint8) {
        uint8 index = getSymbolIndex(symbolName);
        require(index > 0);
        return index;
    }
    ...
    function buyToken(string symbolName, uint priceInWei, uint amount) public{
        ...
        if (tokens[tokenNameIndex].amountSellPrices == 0 || tokens[tokenNameIndex].curSellPrice > priceInWei) {
            ...
            // also need to deduct exchange fee
            ...
            while (whilePrice <= priceInWei && amountNecessary > 0) {//we start with the smallest sell price.
                ...
                while (offers_key <= tokens[tokenNameIndex].sellBook[whilePrice].offers_length && amountNecessary > 0) {//and the first order (FIFO)
                    ...
                    if (volumeAtPriceFromAddress <= amountNecessary) {
                        ...
                        tokenBalanceForAddress[msg.sender][tokenNameIndex] += adjustExchangeFee(tokenNameIndex, msg.sender, volumeAtPriceFromAddress);
                        ...
                        // we need to deduct from buyer's amount
                        ...
                        tokenBalanceForAddress[msg.sender][tokenNameIndex] += adjustExchangeFee(tokenNameIndex, msg.sender, amountNecessary);
            ...
            while (whilePrice >= priceInWei && amountNecessary > 0) {//we start with the highest buy price.
                ...
                while (offers_key <= tokens[tokenNameIndex].buyBook[whilePrice].offers_length && amountNecessary > 0) {//and the first order (FIFO)
                    ...
                    if (volumeAtPriceFromAddress <= amountNecessary) {
                        ...
                        //this guy offers less or equal the volume that we ask for, so we use it up completely.
                        tokenBalanceForAddress[tokens[tokenNameIndex].buyBook[whilePrice].offers[offers_key].who][tokenNameIndex] += adjustExchangeFee(tokenNameIndex, tokens[tokenNameIndex].buyBook[whilePrice].offers[offers_key].who, volumeAtPriceFromAddress);
                        ...
                        balanceEthForAddress[msg.sender] += total_amount_ether_necessary;
                        tokenBalanceForAddress[tokens[tokenNameIndex].buyBook[whilePrice].offers[offers_key].who][tokenNameIndex] += adjustExchangeFee(tokenNameIndex, tokens[tokenNameIndex].buyBook[whilePrice].offers[offers_key].who, amountNecessary);
    ...
    function adjustExchangeFee (uint8 tokenNameIndex, address account,  uint amount) internal returns (uint) {
        require(amount > exchangeFee);
        require(amount - exchangeFee < amount);
        require(tokenBalanceForAddress[address(this)][tokenNameIndex] + exchangeFee > tokenBalanceForAddress[address(this)][tokenNameIndex]);
        // Add the token as fee to exchange account
        tokenBalanceForAddress[address(this)][tokenNameIndex] += exchangeFee;
        TransactionFeeApplied(tokenNameIndex, account, amount, exchangeFee, now);
